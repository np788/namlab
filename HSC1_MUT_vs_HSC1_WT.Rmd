...
```{r include=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
gostres <- gost(query = rownames(upreg), organism = "hsapiens")
...
```{r include=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
gostres <- gost(query = rownames(downreg), organism = "hsapiens")
gostplot(gostres, capped = FALSE, interactive = TRUE)
```
...
```{r include=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
gse_up <- gseGO(geneList=upreg_list, 
             ont ="ALL", 
             keyType = "SYMBOL", 
             minGSSize = 3, 
             maxGSSize = 800, 
...
gse_down <- gseGO(geneList=downreg_list, 
             ont ="ALL", 
             keyType = "SYMBOL", 
             minGSSize = 3, 
             maxGSSize = 800, 
...
```{r include=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
require(DOSE)
dotplot(gse_up, showCategory=10, split=".sign") + facet_grid(.~.sign)
```
...
```{r include=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
require(DOSE)
dotplot(gse_down, showCategory=10, split=".sign") + facet_grid(.~.sign)
```
...
```{r include=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
ridgeplot(gse_up) + labs(x = "enrichment distribution")
```

### Downregulated

```{r include=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
ridgeplot(gse_down) + labs(x = "enrichment distribution")
```
...
gseaplot2(gse_up, geneSetID=1:10)
```
...
```{r include=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
#gseaplot(gse, by = "all", title = gse$Description[3], geneSetID = 3)
gseaplot2(gse_down, geneSetID=1:10)
```
...
kk2_up <- gseKEGG(geneList  = upreg_list,
...
kk2_down <- gseKEGG(geneList = downreg_list,
               organism     = kegg_organism,
               minGSSize    = 3,
               maxGSSize    = 800,
               pvalueCutoff = 0.05,
...
```{r include=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
dotplot(kk2_up, showCategory = 10, title = "Enriched Pathways" , split=".sign") + facet_grid(.~.sign)
```

### Downregulated

```{r include=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
dotplot(kk2_down, showCategory = 10, title = "Enriched Pathways" , split=".sign") + facet_grid(.~.sign)
```
...
fgseaRes <- fgseaMultilevel(pathways=pathwaysH, stats=upreg_list, scoreType = "pos")
...
fgseaRes <- fgseaMultilevel(pathways=pathwaysH, stats=downreg_list, scoreType = "neg")
...
fgseaRes <- fgseaMultilevel(pathways=pathwaysB, stats=upreg_list, scoreType = "pos")
...
fgseaRes <- fgseaMultilevel(pathways=pathwaysB, stats=downreg_list, scoreType = "neg")
...
fgseaRes <- fgseaMultilevel(pathways=pathwaysC, stats=upreg_list, scoreType = "pos")
...
fgseaRes <- fgseaMultilevel(pathways=pathwaysC, stats=downreg_list, scoreType = "neg")
